...
describe('validateCheckoutSubmission', () => {
  ...
  it('accepts a well-formed order meeting the 2-kit minimum', () => {
    const { result, data } = validateCheckoutSubmission({ ...baseSubmission, items: baseItems });
    expect(result.valid).toBe(true);
  ...
  it('accepts an order with no phone (optional) and no address line 2 (optional)', () => {
    const { result } = validateCheckoutSubmission({ ...baseSubmission, items: baseItems });
    expect(result.valid).toBe(true);
  });
  ...
  it('rejects an empty cart', () => {
    const { result } = validateCheckoutSubmission({ ...baseSubmission, items: [] });
    expect(result.valid).toBe(false);
  });
});
